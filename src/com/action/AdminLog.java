...
public class AdminLog extends ActionSupport {
	...
	/**
	 * AdminLog通过Domitory_BuildingID查询寝室DomitoryDao，根据Building_Name查询BuildingDao
	 */
	...
	public String execute() throws Exception {
		...
		//查询楼宇（order by Building_Name）
		buildinglist=new BuildingDao().GetList("","Building_Name");
//		System.out.println(BuildingID);
		//查询条件
		String strWhere="1=1 ";//方便作为字符串执行sql语句
		if(!(isInvalid(BuildingID)))
		{
			strWhere+=" and Domitory_BuildingID='"+BuildingID+"'";
		}
		else{
			strWhere+=" and 1=2";//合并两个表之后1=2相当于外键约束
		}
		//查询寝室（order by Domitory_Name）
